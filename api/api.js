...
export default async function handler(req, res) {
    ...
    try {
        ...
        const offerHashFromPayload = payload.offer_hash || payload.offerHash || payload.offer || (payload.produtos && payload.produtos[0]?.offer_hash) || config.default_offer_hash;
        const productHashFromPayload = payload.product_hash || payload.productHash || payload.product || (payload.produtos && payload.produtos[0]?.product_hash) || config.default_product_hash;
        const defaultGatewayTitle = payload.product_name || payload.gateway_title || (payload.produtos && payload.produtos[0]?.nome) || 'Com ter a casa refrigerada o verao';

        // Montar payload para TriboPay
        const tribopayPayload = {
            api_token: config.api_token,
            amount: amountCents,
            offer_hash: offerHashFromPayload || config.default_offer_hash,
        ...
        if (payload.produtos && Array.isArray(payload.produtos)) {
            for (const produto of payload.produtos) {
                const produtoHash = produto.product_hash || produto.productHash || productHashFromPayload || config.default_product_hash;
                const tituloProduto = produto.nome || produto.title || defaultGatewayTitle || 'Produto';
                const quantidadeProduto = produto.quantidade || produto.quantity || 1;
                const precoProduto = toCents(produto.preco ?? produto.price ?? payload.amount);

                tribopayPayload.cart.push({
                    product_hash: produtoHash,
                    title: tituloProduto,
                    price: precoProduto,
            ...
            // Produto padrão
            tribopayPayload.cart.push({
                product_hash: productHashFromPayload || config.default_product_hash,
                title: defaultGatewayTitle || 'Produto',
                price: amountCents,
                quantity: 1,
                operation_type: config.default_operation_type || 1,
                tangible: config.default_tangible || true
